...
```sql
SELECT COUNT(*) FROM sales_projects.retail_sales;
SELECT COUNT(DISTINCT customer_id) FROM sales_projects.retail_sales;
SELECT DISTINCT category FROM sales_projects.retail_sales;

SELECT * FROM sales_projects.retail_sales
WHERE 
...
#### RESULTS
	...
	4. NULL transaction- there was no null value and no value to delete Recall 13 null values were earlier removed upon importing dataset
...
```
#### Result: £6,830.00 was made as sales revenue on 05-11-2022 
...
```
#### Result age of customer who purchased beauty products was between 18 and 64 with the average of 40.42 years
...
6. **What are the revenue made by each gender in each category.**:
```sql
SELECT 
    category,
    gender,
